...
def start(event=None):
    ...
    def return_to_main(event=None):
        for button in buttons:
            canvas.delete(button)
        show_initial_screen()

    back_button = canvas.create_image(80/1280*screen_width, 30/720*screen_height, anchor="nw", image=back_image_tk)
    canvas.tag_bind(back_button, "<Button-1>", return_to_main)

def show_initial_screen():
    global shadow, title, start_button
    shadow = canvas.create_image(screen_width/2, screen_height/3, anchor="center", image=shadow_image)
    title = canvas.create_image(screen_width/2, screen_height/3, anchor="center", image=title_image)
